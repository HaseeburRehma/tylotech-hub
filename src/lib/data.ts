 ...
 *
 * Reads through the RLS-scoped Supabase server client; when the backend isn't configured
 * — or a table hasn't been seeded yet — it falls back to the mock layer so the UI is never
 * blank. Once real rows exist, they take over automatically.
 */
...
export async function getClient(id: string): Promise<Client | null> {
  const sb = createClient();
  if (!sb) return mock.CLIENTS.find((c) => c.id === id) ?? null;
...
export async function getKpis(clientId: string | null): Promise<Kpi[]> {
  const sb = createClient();
  if (!sb || !clientId) return mock.kpisFor(clientId ?? "nordic");
  const { data } = await sb.from("kpis").select("*").eq("client_id", clientId);
  if (!data?.length) return mock.kpisFor(clientId);
  return data as Kpi[];
}

export async function listProjects(clientId?: string | null): Promise<Project[]> {
  const sb = createClient();
  if (!sb) return mock.PROJECTS;
  let q = sb.from("projects").select("*").order("created_at", { ascending: false });
  if (clientId) q = q.eq("client_id", clientId);
  const { data } = await q;
  if (!data?.length) return clientId ? [] : mock.PROJECTS;
  return data as Project[];
}

export async function listUpdates(clientId: string | null): Promise<Update[]> {
  const sb = createClient();
  if (!sb || !clientId) return mock.UPDATES;
  const { data } = await sb
    .from("updates")
    .select("*")
    .eq("client_id", clientId)
    .order("created_at", { ascending: false });
  if (!data?.length) return mock.UPDATES;
  return data as Update[];
}

export async function listDocuments(clientId: string | null): Promise<DocItem[]> {
  const sb = createClient();
  if (!sb || !clientId) return mock.DOCUMENTS;
  const { data } = await sb
    .from("documents")
    .select("*")
    .eq("client_id", clientId)
    .order("created_at", { ascending: false });
  if (!data?.length) return mock.DOCUMENTS;
  return data as DocItem[];
}

export async function getSeries(clientId: string | null): Promise<SeriesPoint[]> {
  const sb = createClient();
  if (!sb || !clientId) return mock.SERIES;
  const { data } = await sb
    .from("metric_points")
    .select("date,spend,leads,roas")
    .eq("client_id", clientId)
    .order("date", { ascending: true });
  if (!data?.length) return mock.SERIES;
...
export async function listMessages(clientId: string | null): Promise<Message[]> {
  const sb = createClient();
  if (!sb || !clientId) return mock.MESSAGES;
  const { data } = await sb
    .from("messages")
    .select("*")
    .eq("client_id", clientId)
    .order("created_at", { ascending: true });
  if (!data?.length) return mock.MESSAGES;
...
export async function listTeamMembers(): Promise<TeamMember[]> {
  const sb = createClient();
  if (!sb) return mock.TEAM.map((t) => ({ id: t.id, name: t.name, role: t.role }));
  const { data } = await sb.from("users").select("id,name,role").in("role", ["admin", "team"]);
  if (!data?.length) return mock.TEAM.map((t) => ({ id: t.id, name: t.name, role: t.role }));
